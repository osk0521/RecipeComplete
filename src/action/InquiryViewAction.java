...
public class InquiryViewAction implements Action {
	...
	public void execute(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		// 문의사항 목록을 가져오기 위한 Dao 생성
		CustomerServiceDao csDao = new CustomerServiceDao();
		if(loginId != null) {
			// 로그인되어있는 아이디가 있다면 페이지 header에 로그인 유저 정보를 표시하기 위해 로그인 유저 프로필 정보 뽑아오기
			ProfileDto myProfile = new ProfileDao().getProfileById(loginId);
			request.setAttribute("myProfile", myProfile);
			// 로그인 유저의 문의사항을 가져옴
			ArrayList<InquiryDto> inquiryList = csDao.getInquiry(loginId);
			request.setAttribute("inquiryList", inquiryList);
		}
		
		request.getRequestDispatcher("Recipe_Inquiry.jsp").forward(request, response);
	}
}
