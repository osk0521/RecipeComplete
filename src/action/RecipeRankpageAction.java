...
import dao.RecipeRankBoardDao;
import dto.ProfileDto;
import dto.RecentRecipeDto;
import vo.RecipeRankVo;
import vo.RecipeUserRankVo;

public class RecipeRankpageAction implements Action {
	...
	public void execute(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		RecipeRankBoardDao recipeRankBoardDao = new RecipeRankBoardDao();
		
		ArrayList<RecipeRankVo> top100RecipeList = recipeRankBoardDao.getTop100Recipe(ptype);
		ArrayList<RecipeUserRankVo> top100UserList = recipeRankBoardDao.getTop100User(ptype);
		ArrayList<String> top100Searchword = recipeRankBoardDao.getTop100Searchword(ptype);
		
		request.setAttribute("top100RecipeList", top100RecipeList);
		request.setAttribute("top100UserList", top100UserList);
		request.setAttribute("top100Searchword", top100Searchword);
		RequestDispatcher rd = request.getRequestDispatcher("Recipe_Ranking.jsp");
