...
public class MyPageRecipeNoteViewAction implements Action {
	@Override
	public void execute(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		if(loginId != null) {
			...
			// 로그인 계정의 레시피 노트를 받아올 수 있는 DAO객체 생성
			MyPageRecipeNoteDao recipeNoteDao = MyPageRecipeNoteDao.getMyRecipeNoteDao();
			// 로그인 계정의 레시피 노트 수를 기반으로 페이지 수를 가져오기
			lastPageNum = recipeNoteDao.getRecipeNotePageNum(loginId);
			// 로그인 계정의 레시피 노트를 저장할 리스트 생성
			ArrayList<MyPageRecipeNoteDto> myRecipeNoteList = recipeNoteDao.getMyRecipeNote(loginId, pageNum);
			request.setAttribute("lastPageNum", lastPageNum);
			request.setAttribute("myRecipeNoteList", myRecipeNoteList);
			...
			if(pageNum>lastPageNum && lastPageNum!=0) {
				...
				// 페이지 이동을 위한 RequestDispatcher 객체 생성
				request.getRequestDispatcher("Recipe_MyPage_RecipeNote.jsp").forward(request, response);
			}
