...
<head>
	...
	<link href="Recipe_CSS/Recipe_Inquiry_Write.css" rel="stylesheet">
	...
	<script src="se2823/js/HuskyEZCreator.js" charset="utf-8"></script>
	<script>
		let oEditors = [];
		...
		$(function() {
			nhn.husky.EZCreator.createInIFrame({
				oAppRef : oEditors,
				elPlaceHolder: "inquiry_write_content",
				sSkinURI: "se2823/SmartEditor2Skin.html",
				fCreator: "createSEditor2"
			});
			
// 			 /* 저장 */
// 	        $("#btn_yg").click(function(e) {
// 	        	oEditors.getById["inquiry_write_content"].exec("UPDATE_CONTENTS_FIELD", []);
// 	            /* 밸리데이션체크 및 액션 */
// 	        });

			// submit
			$("#button_reg_inquiry").click(function() {
				oEditors.getById["inquiry_write_content"].exec("UPDATE_CONTENTS_FIELD", []);
				$("form").submit();
			});
		});
...
<!-- 문의사항 작성 -->
			<form action="Controller" method="post">
				<input type="hidden" name="command" value="inquiry_write"/>
				<input type="text" name="inquiry_title" id="inquiry_write_title" placeholder="제목">
				<div class="blank_space"></div>
<!-- 여기에 삽입 -->
<!-- <button type="button" id="btn_yg">YGYG</button> -->
				<div id="div_inquiry_content">
					<textarea name="inquiry_content" id="inquiry_write_content" placeholder="내용">
쇼핑몰 문의의 경우 아래 양식에 맞게 항목을 기재해주시면 빠른 업무처리가 가능합니다.
<br/>휴일간 문의사항은 휴무일 이후 순차적으로 처리해드리겠습니다.
<br/>주문번호 : 
<br/>상품명 : 
<br/>주문자 이메일 :
<br/>주문자 전화번호 :
...
<!-- /여기에 삽입 -->
				<div class="blank_space"></div>
				<div id="div_inquiry_regbar">
					<button type="button" id="button_reg_inquiry">등록</button>
					<button type="button" id="button_reg_cancel">취소</button><!-- inquiry_view로 이동 -->
				</div>
			</form>
<!-- /문의사항 작성 -->
		</div>
