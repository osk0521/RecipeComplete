...
<%@page import="dto.MyPageRecipeNoteDto"%>
...
<%
	...
	ArrayList<MyPageRecipeNoteDto> myRecipeNoteList = null;
	...
	if(loginId != null) {
		...
		// 로그인 계정의 레시피 노트 정보
		myRecipeNoteList = (ArrayList<MyPageRecipeNoteDto>)request.getAttribute("myRecipeNoteList");
	}
...
<head>
	...
	<link href="Recipe_CSS/Recipe_MyPage_RecipeNote.css" rel="stylesheet">
		...
		<div id="div_header_inner">
			...
			<div id="div_header_button">
				...
				<div id="div_recipeWrite_button" class="fl">
					...
					<% } else { %>
						<a move_write_recipe()onclick="move_write_recipe();">
							<img src="https://recipe1.ezmember.co.kr/img/tmn_write.png"/>
						</a>
					<% } %>
				</div>
...
<!-- /마이페이지 헤더 -->
<!-- 메인컨텐츠(레시피 노트) -->
			<div id="div_mypage_note_panel">
					<% if(loginId != null) { %>
						<% if(myRecipeNoteList.isEmpty()) { %>
							<div id="div_mypage_note_body" style="text-align:center; margin:22px 10px; padding:0;">
								<div id="div_mypage_note_none">
									<img src="https://recipe1.ezmember.co.kr/img/none_note.png"/>
									<p>
										잊어버리기 전에 노트를 이용하세요!
									</p>
									레시피와 관련된 메모나 팁을 레시피 하단 &lt;노트&gt;를 통해 쉽게 남길 수 있습니다.
								</div>
							</div>
						<% } else { %>
							<div id="div_mypage_note_body">
								<% for(MyPageRecipeNoteDto noteObj : myRecipeNoteList) { %>
									<%
										pageContext.setAttribute("recipeId", noteObj.getRecipeId());
										pageContext.setAttribute("recipeTitle", noteObj.getRecipeTitle());
										pageContext.setAttribute("recipeThumbanil", noteObj.getRecipeThumbnail());
										pageContext.setAttribute("noteContent", noteObj.getNoteContent());
										pageContext.setAttribute("noteWritedate", noteObj.getNoteWritedate().split(" ")[0]);
									%>
									<div class="div_mypage_note_item">
										<input type="hidden" value="${recipeId}" class="input_recipe_id"/>
										<div class="mypage_note_header">
											<a>
												<img src="https://recipe1.ezmember.co.kr/img/icon_del.png"/>
											</a>
											<a>
												<img src="https://recipe1.ezmember.co.kr/img/icon_modify.png"/>
											</a>
											${noteWritedate}
										</div>
										<div class="mypage_note_body">${noteContent}</div>
										<div class="mypage_note_footer">
											<a href="Controller?command=view_recipe_detail&recipeID=${recipeId}"> <!-- 레시피 상세페이지 -->
												<span>${recipeTitle}</span>
												<img src="${recipeThumbanil}"/>
											</a>
										</div>
									</div>
								<% } %>
										...
										<div id="div_mypage_page_bundle">
											<% if(startPageNum != 1) { %>
												<div class="div_mypage_page_button">
													<a href="Controller?command=mypage_recipenote_view&page=<%=startPageNum-3%>">&lt;&lt;</a>
												</div>
											<% } %>
											<% if(pageNum > 1) { %>
												<div class="div_mypage_page_button">
													<a href="Controller?command=mypage_recipenote_view&page=<%=pageNum-1%>">&lt;</a>
												</div>
											<% } %>
											<% for(int i=startPageNum; i<=endPageNum; i++) { %>
												...
												<% if(i == pageNum) { %>
													<div class="div_mypage_page_button">
														<a href="Controller?command=mypage_recipenote_view&page=<%=i%>" class="button_active"><%=i%></a>
													</div>
												<% } else { %>
													<div class="div_mypage_page_button">
														<a href="Controller?command=mypage_recipenote_view&page=<%=i%>"><%=i%></a>
													</div>
												<% } %>
											<% } %>
											...
											<% if(endPageNum < lastPageNum) { %>
												<div class="div_mypage_page_button">
													<a href="Controller?command=mypage_recipenote_view&page=<%=endPageNum+1%>">&gt;&gt;</a>
												</div>
											<% } %>
										</div>
									</div>
...
<!-- 레시피 노트 수정영역 -->
				<form action="Controller" method="get">
					<input type="hidden" name="command" value="mypage_recipenote_modify"/>
					<input type="hidden" id="input_modify_recipe_id" name="modify_recipe_id"/>
					<div id="div_mypage_note_update" class="div_mypage_extra">
						<div id="div_mypage_note_update_area" class="div_mypage_extra_area">
							<div id="div_mypage_note_update_box" class="div_mypage_extra_box">
								<div id="div_mypage_note_update_header" class="div_mypage_extra_box_title">
									<h4 class="h4_box_header_title" id="note_recipe_id"></h4>
									<button type="button" class="button_box_close">
										<img src="https://recipe1.ezmember.co.kr/img/btn_close.gif" alt="닫기"/>
									</button>
								</div>
								<div id="div_mypage_note_update_body" class="div_mypage_extra_box_body">
									<textarea id="textarea_note_content" name="content"></textarea> <!-- 기존에 저장되어있던 레시피 노트의 text()를 가져옴 -->
								</div>
								<div id="div_mypage_note_update_footer">
