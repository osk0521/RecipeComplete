...
<%@page import="dto.RecentReceiveCommentOfFollowingChefDto"%>
<%@page import="dto.RecentRecipeOfFollowingChefDto"%>
<%@page import="dto.MyPageFollowingChefDto"%>
...
<%
	...
	// 타인 계정의 팔로잉 쉐프 목록에 필요한 리스트 초기화
	ArrayList<MyPageFollowingChefDto> otherFollowingChefList = (ArrayList<MyPageFollowingChefDto>)request.getAttribute("otherFollowingChefList");
...
<html>
<head>
	<!-- 마이페이지 본인계정일 때의 팔로우중인 쉐프 페이지(팔로잉 쉐프가 아무도 없을 때) -->
	<meta charset="UTF-8">
	...
	<link href="Recipe_CSS/Recipe_MyPage_FollowingChef.css" rel="stylesheet">
...
<!-- 마이페이지 헤더 -->
			<div id="div_mypage_header">
				...
				<div class="div_mypage_header_button">
					<a href="Controller?command=mypage_comment_view&uId=<%=otherUid%>">
						<span class="glyphicon glyphicon-star"></span>
						요리후기
					</a>
				</div>
				<div class="div_mypage_header_button">
					<a href="Controller?command=mypage_followingchef_view&uId=<%=otherUid%>" class="button-active">
...
<!-- /마이페이지 헤더 -->
<!-- 메인컨텐츠(마이페이지 팔로우쉐프) -->
			<div id="div_mypage_chef_panel">
				<div id="div_mypage_chef_sort">
					<div id="div_mypage_chef_sort1">
						<div>
							<a class="open_active">소식받는 쉐프</a>
						</div>
					</div>
					<div id="div_mypage_chef_search_bar">
						<form action="Controller" method="get">
							<input type="hidden" name="command" value="mypage_followingchef_view"/>
							<input type="hidden" name="uId" value="<%=otherUid%>"/>
							<span class="glyphicon glyphicon-search"></span>
				...
				<% if(loginId != null) { %>
					<% if(otherFollowingChefList.isEmpty()) { %>
						<div id="div_mypage_chef_body" style="text-align:center; margin:22px 10px; padding:0;">
							<div id="div_mypage_chef_none">
								<img src="https://cdn-icons-png.flaticon.com/512/189/189627.png"/>
								<p>
									소식받는 쉐프가 없으신가요?
								</p>
								관심있는 쉐프의 소식을 실시간으로 받아보세요!
							</div>
						</div>
					<% } else { %>
						<div id="div_mypage_chef_body">
							<% for(MyPageFollowingChefDto followingChefObj : otherFollowingChefList) { %>
								<%
									String followingChefUid = followingChefObj.getFollowingChefUid();
									String followingChefNickname = followingChefObj.getFollowingChefNickname();
									String followingChefProfileImage = followingChefObj.getFollowingChefProfileImage();
									int recipeQtyOfFollowingChef = followingChefObj.getRecipeQtyOfFollowingChef();
									int recipeLikeQtyOfFollowingChef = followingChefObj.getRecipeLikeQtyOfFollowingChef();
									int recipeHitQtyOfFollowingChef = followingChefObj.getRecipeHitQtyOfFollowingChef();
									ArrayList<RecentRecipeOfFollowingChefDto> recentRecipeListOfFollowingChef = followingChefObj.getRecentRecipeListOfFollowingChef();
									ArrayList<RecentReceiveCommentOfFollowingChefDto> recentReceiveCommentListOfFollowingChef = followingChefObj.getRecentReceiveCommentListOfFollowingChef();
									int followerQtyOfFollowingChef = followingChefObj.getFollowerQtyOfFollowingChef();
								%>
								<div class="div_mypage_chef_item">
									<div class="div_mypage_chef_img">
										<a href="Controller?command=mypage_recipe_view&uId=<%=followingChefUid%>"> <!-- 유저 마이페이지 -->
											<img src="<%=followingChefProfileImage%>"/>
										</a> 
									</div>
									<div class="div_mypage_chef_content">
										<div class="div_mypage_chef_user">
											<div class="div_mypage_chef_user_name">
												<a href="Controller?command=mypage_recipe_view&uId=<%=followingChefUid%>"><%=followingChefNickname%></a>
											</div>
											<div class="div_mypage_chef_user_figures">
												레시피
												<b><%=recipeQtyOfFollowingChef%></b>
												레시피 좋아요
												<b><%=recipeLikeQtyOfFollowingChef%></b>
												레시피 조회수
												<b><%=recipeHitQtyOfFollowingChef%></b>
												소식받는 수
												<b><%=followerQtyOfFollowingChef%></b>
											</div>
										</div>
										<div class="div_mypage_chef_recipe_img">
											<% for(RecentRecipeOfFollowingChefDto recipeObj : recentRecipeListOfFollowingChef) { %>
												<a href="Controller?command=view_recipe_detail&recipeID=<%=recipeObj.getRecipeId()%>"> <!-- 레시피 상세링크 -->
													<img src="<%=recipeObj.getThumbnail()%>"/>
												</a>
											<% } %>
										</div>
										<div class="div_mypgae_chef_comments">
											<% for(RecentReceiveCommentOfFollowingChefDto commentObj : recentReceiveCommentListOfFollowingChef) { %>
												<div class="div_mypgae_chef_comment">
													<div>
														<a href="Controller?command=mypage_recipe_view&uId=<%=commentObj.getCommentWriterUid()%>">
															<b><%=commentObj.getCommentWriterNickname()%></b>
														</a>
													</div>
													<div>
														<a><%=commentObj.getCommentContent()%></a>
													</div>
												</div>
											<% } %>
										</div>
							...
							<div id="div_mypage_page_panel">
									...
									<div id="div_mypage_page_bundle">
										<% if(startPageNum != 1) { %>
											<div class="div_mypage_page_button">
												<a href="Controller?command=mypage_followingchef_view&uId=<%=otherUid%>&page=<%=startPageNum-5%>">&lt;&lt;</a>
											</div>
										<% } %>
										<% if(pageNum != 1) { %>
											<div class="div_mypage_page_button">
												<a href="Controller?command=mypage_followingchef_view&uId=<%=otherUid%>&page=<%=pageNum-1%>">&lt;</a>
											</div>
										<% } %>
										<% for(int i=startPageNum; i<=endPageNum; i++) { %>
											...
											<% if(i == pageNum) { %>
												<div class="div_mypage_page_button">
													<a href="Controller?command=mypage_followingchef_view&uId=<%=otherUid%>&page=<%=i%>" class="button_active"><%=i%></a>
												</div>
											<% } else { %>
												<div class="div_mypage_page_button">
													<a href="Controller?command=mypage_followingchef_view&uId=<%=otherUid%>&page=<%=i%>"><%=i%></a>
												</div>
											<% } %>
										<% } %>
								...
								<% } else { %>
									<div id="div_mypage_page_bundle">
										<% if(startPageNum != 1) { %>
											<div class="div_mypage_page_button">
												<a href="Controller?command=mypage_followingchef_view&uId=<%=otherUid%>&searchWord=<%=searchWord%>&page=<%=startPageNum-5%>">&lt;&lt;</a>
											</div>
										<% } %>
										<% if(pageNum != 1) { %>
											<div class="div_mypage_page_button">
												<a href="Controller?command=mypage_followingchef_view&uId=<%=otherUid%>&searchWord=<%=searchWord%>&page=<%=pageNum-1%>">&lt;</a>
											</div>
										<% } %>
										<% for(int i=startPageNum; i<=endPageNum; i++) { %>
											<% if(i > lastPageNum) break; %>
											<% if(i == pageNum) { %>
												<div class="div_mypage_page_button">
													<a href="Controller?command=mypage_followingchef_view&uId=<%=otherUid%>&searchWord=<%=searchWord%>&page=<%=i%>" class="button_active"><%=i%></a>
												</div>
											<% } else { %>
												<div class="div_mypage_page_button">
													<a href="Controller?command=mypage_followingchef_view&uId=<%=otherUid%>&searchWord=<%=searchWord%>&page=<%=i%>"><%=i%></a>
												</div>
											<% } %>
										<% } %>
										<% if(pageNum != lastPageNum) { %>
