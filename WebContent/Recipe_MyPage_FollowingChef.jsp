...
<%
	...
	// 로그인 계정의 팔로잉 쉐프 목록에 필요한 변수 선언
	ArrayList<MyPageFollowingChefDto> myFollowingChefList = null;
	...
	if(loginId != null) {
		...
		// 로그인 계정의 팔로잉 쉐프 목록 리스트
		myFollowingChefList = (ArrayList<MyPageFollowingChefDto>)request.getAttribute("myFollowingChefList"); 
	}
...
<!-- 마이페이지 헤더 -->
			<div id="div_mypage_header">
				...
				<div class="div_mypage_header_button">
					<a href="Controller?command=mypage_followingchef_view" class="button-active">
...
<!-- 메인컨텐츠(마이페이지 팔로우쉐프) -->
			<div id="div_mypage_chef_panel">
				...
				<% if(loginId != null) { %>
					<% if(myFollowingChefList.isEmpty()) { %>
					...
					<% } else { %>
						<div id="div_mypage_chef_body">
							<% for(MyPageFollowingChefDto followingChefObj : myFollowingChefList) { %>
								<%
										...
										<div class="div_mypgae_chef_comments">
											<% for(RecentReceiveCommentOfFollowingChefDto commentObj : recentReceiveCommentListOfFollowingChef) { %>
												<div class="div_mypgae_chef_comment">
													...
													<div>
														<a href="#"><%=commentObj.getCommentContent()%></a>
													</div>
												</div>
											<% } %>
										</div>
							...
							<div id="div_mypage_page_panel">
									...
									<div id="div_mypage_page_bundle">
										<% if(startPageNum != 1) { %>
											<div class="div_mypage_page_button">
												<a href="Controller?command=mypage_followingchef_view&page=<%=startPageNum-5%>">&lt;&lt;</a>
											</div>
										<% } %>
										<% if(pageNum != 1) { %>
											<div class="div_mypage_page_button">
												<a href="Controller?command=mypage_followingchef_view&page=<%=pageNum-1%>">&lt;</a>
											</div>
										<% } %>
										<% for(int i=startPageNum; i<=endPageNum; i++) { %>
											...
											<% if(i == pageNum) { %>
												<div class="div_mypage_page_button">
													<a href="Controller?command=mypage_followingchef_view&page=<%=i%>" class="button_active"><%=i%></a>
												</div>
											<% } else { %>
												<div class="div_mypage_page_button">
													<a href="Controller?command=mypage_followingchef_view&page=<%=i%>"><%=i%></a>
												</div>
											<% } %>
										<% } %>
								...
								<% } else { %>
									<div id="div_mypage_page_bundle">
										<% if(startPageNum != 1) { %>
											<div class="div_mypage_page_button">
												<a href="Controller?command=mypage_followingchef_view&searchWord=<%=searchWord%>&page=<%=startPageNum-5%>">&lt;&lt;</a>
											</div>
										<% } %>
										<% if(pageNum != 1) { %>
											<div class="div_mypage_page_button">
												<a href="Controller?command=mypage_followingchef_view&searchWord=<%=searchWord%>&page=<%=pageNum-1%>">&lt;</a>
											</div>
										<% } %>
										<% for(int i=startPageNum; i<=endPageNum; i++) { %>
											<% if(i > lastPageNum) break; %>
											<% if(i == pageNum) { %>
												<div class="div_mypage_page_button">
													<a href="Controller?command=mypage_followingchef_view&searchWord=<%=searchWord%>&page=<%=i%>" class="button_active"><%=i%></a>
												</div>
											<% } else { %>
												<div class="div_mypage_page_button">
													<a href="Controller?command=mypage_followingchef_view&searchWord=<%=searchWord%>&page=<%=i%>"><%=i%></a>
												</div>
											<% } %>
										<% } %>
										<% if(pageNum != lastPageNum) { %>
