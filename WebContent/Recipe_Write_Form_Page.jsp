...
<%
	...
	int newRecipeID = (Integer) request.getAttribute("newRecipeID");
	System.out.print(newRecipeID);
...
<!-- /페이지 상단 메뉴바 -->
	<div class="container">
		...
		<form action="Controller?command=recipe_write&recipeID=<%=newRecipeID%>" method="post">
			<div class="cont_box">
				<div class="div_main_photo_upload">
					...
					<img id="main_photo_holder" style = "display: none;"> <!--  ="upload/${img_src}" -->
				</div>
		  		<div class="cont_line">
					<p class="cont_tit">레시피 제목</p>
					<input style="width: 610px;"type="text" id="cok_title" name="title" class="form-control" placeholder="예) 소고기 미역국 끓이기" required="required">
				</div>
		  		<div class="cont_line">
					<p class="cont_tit">요리소개</p>
					<textarea id="cok_intro" class="form-control step_cont" name="introduce" spellcheck="false" placeholder="이 레시피의 탄생배경을 적어주세요. 예) 남편의 생일을 맞아 소고기 미역국을 끓여봤어요. 어머니로부터 배운 미역국 레시피를 남편의 입맛에 맞게 고안했습니다."></textarea>
		  		</div>
		  		<div class="cont_line">
		  			<p class="cont_tit">동영상</p>
		  			<textarea class="form-control step_cont" id="video_link" name="video_link" placeholder="동영상이 있으면 주소를 입력하세요.(Youtube,네이버tvcast,다음tvpot 만 가능) 예)http://youtu.be/lA0Bxo3IZmM"></textarea>
	...
	<!-- 				<span class="guide" style="margin: -22px 0 0 146px;">분류를 바르게 설정해주시면, 이용자들이 쉽게 레시피를 검색할 수 있어요.</span> -->
		  	...
		  	<div class="cont_box">
		  		...
		  		<div class="material_group">
		  			<div id="div_material_group">
			  			<li id="li_material_group_1" class="li_material_group">
			  				<p class="cont_tit6">
			  					<input type="text" id="material_group_title_1" name="material_group_title_1" value="재료" class="categorized form-control" name="bundle_name">
							</p>
							<ul class="material_group_list" id="material_group_list_1">
							...
							<div class="btn_add">
								<button type="button" class="btn btn-default add_ingredi" id="add_ingredi_1">
			...
			<div style="display:inline-block">
				...
				<div id="div_work_upload_<%=j %>" class="complete_pic">
					...
					<img id="last_photo_holder_<%=j%>" name="last_image_<%=j%>" class="step_photo_holder" src="" style="display: none;">
				</div>
				<%} %>
			</div>
	  	</div>
		  	...
		  	<div class="cont_box" >
				...
				<ul class="tagit">
					<li class="tagit-new">
						<input type="text" class="new-tag" name="tags" autocomplete="off">
					</li>
				</ul>
				<span class="tag-tip1">
					주재료, 목적, 효능, 대상 등을 태그로 남겨주세요. (띄어쓰기로 분리 해주세요)
					<span class="tag-tip2">
			...
			<div class="save_btm">
				...
				<input type="hidden" name="recipeID" id="recipeID" value="<%=newRecipeID %>"/> <!-- 새 레시피 아이디 -->
			</div>
		</form>
	</div>
</div>
