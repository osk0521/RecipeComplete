...
<%@page import="dto.MyPageFollowingRecipeDto"%>
...
<%
	...
	// 로그인 계정의 레시피 노트 정보에 필요한 변수 선언
	ArrayList<MyPageFollowingRecipeDto> myFollowingRecipeList = null;
	...
	if(loginId != null) {
		...
		// 로그인 계정의 레시피 노트 정보
		myFollowingRecipeList = (ArrayList<MyPageFollowingRecipeDto>)request.getAttribute("myFollowingRecipeList");
	}
...
<html>
<head>
	<!-- 마이페이지 본인계정일 때의 레시피 좋아요 페이지(정렬방식:표) -->
	<meta charset="UTF-8">
	...
	<link href="Recipe_CSS/Recipe_MyPage_FollowingRecipe.css" rel="stylesheet">
...
<!-- /마이페이지 헤더 -->
			<div id="div_mypage_followrecipe_panel">
				<form action="Controller" method="post">
					<input type="hidden" name="command" value="mypage_followingrecipe_delete"/>
<!-- 팔로우레시피 헤더 -->
					<div id="div_mypage_followrecipe_header">
						<span>팔로우 중인 레시피</span>
						<div>
							<div>
								<button type="button" class="sort_active">
							...
							<button type="button" id="button_followrecipe_all_select">전체선택</button>
							<button id="button_followrecipe_delete" onclick="return delete_followrecipe();">삭제</button>
						</div>
					</div>
<!-- /팔로우레시피 헤더 -->
						<% if(loginId != null) { %>
							<% if(myFollowingRecipeList.isEmpty()) { %>
								<div class="div_mypage_followrecipe_body">
									<div id="div_mypage_followrecipe_none">
										<img src="https://recipe1.ezmember.co.kr/img/none_scrap2.png"/>
										<p>
											맘에드는 레시피를 팔로우하세요!
										</p>
										마음에 드는 레시피를 발견하셨다면 <br/>
										레시피의 하트 버튼을 눌러 이 곳에 보관하세요.
									</div>
								</div>
							<% } else { %>
<!-- 메인컨텐츠(마이페이지 팔로우레시피 - 테이블 정렬방식) -->
								<div class="div_mypage_followrecipe_body">
									<div id="div_mypage_followrecipe_content_table">
										<% for(MyPageFollowingRecipeDto followingRecipeObj : myFollowingRecipeList) { %>
											<%
												pageContext.setAttribute("followingRecipeId", followingRecipeObj.getRecipeId());
												pageContext.setAttribute("followingRecipeTitle", followingRecipeObj.getRecipeTitle());
												pageContext.setAttribute("followingRecipeThumbnail", followingRecipeObj.getRecipeThumbnail());
												pageContext.setAttribute("followingRecipeWriterNickname", followingRecipeObj.getWriterNickname());
											%>
												<div class="div_mypage_followrecipe_item">
													<a href="Controller?command=view_recipe_detail&recipeID=${followingRecipeId}" class="a_mypage_followrecipe_img">
														<img src="${followingRecipeThumbnail}"/>
														<div class="div_mypage_followrecipe_text">
															<div class="followrecipe_title">${followingRecipeTitle}</div>
															<div class="followrecipe_writer">by ${followingRecipeWriterNickname}</div>
														</div>
													</a>
													<div class="div_mypage_followrecipe_checkbox">
														<input type="checkbox" name="followrecipe_checkbox" value="${followingRecipeId}">
													</div>
												</div>
										<% } %>
									</div>
...
<!-- /메인컨텐츠(마이페이지 팔로우레시피 - 테이블 정렬방식) -->
<!-- 메인컨텐츠(마이페이지 팔로우레시피 - 선 정렬방식) -->
								<div class="div_mypage_followrecipe_body">
									<div id="div_mypage_followrecipe_content_line">
										<% for(MyPageFollowingRecipeDto followingRecipeObj : myFollowingRecipeList) { %>
											<%
												pageContext.setAttribute("followingRecipeId", followingRecipeObj.getRecipeId());
												pageContext.setAttribute("followingRecipeTitle", followingRecipeObj.getRecipeTitle());
										...
										<div class="div_mypage_followrecipe_item">
											...
											<div class="div_mypage_followrecipe_img">
												<a href="Controller?command=view_recipe_detail&recipeID=${followingRecipeId}">
													<img src="${followingRecipeThumbnail}"/>
												</a>
											</div>
											<div class="div_mypage_followrecipe_text">
												<div class="followrecipe_title">
													<a href="#">${followingRecipeTitle}</a>
												</div>
												<div class="followrecipe_writer">by ${followingRecipeWriterNickname}</div>
											</div>
										</div>
							...
							<div id="div_mypage_page_bundle">
								<% if(startPageNum > 1) { %>
									<div class="div_mypage_page_button">
										<a href="Controller?command=mypage_followingrecipe_view&page=<%=startPageNum-3%>">&lt;&lt;</a>
									</div>
								<% } %>
								<% if(pageNum > 1) { %>
									<div class="div_mypage_page_button">
										<a href="Controller?command=mypage_followingrecipe_view&page=<%=pageNum-1%>">&lt;</a>
									</div>
								<% } %>
								<% for(int i=startPageNum; i<=endPageNum; i++) { %>
									...
									<% if(i == pageNum) { %>
										<div class="div_mypage_page_button">
											<a href="Controller?command=mypage_followingrecipe_view&page=<%=i%>" class="button_active"><%=i%></a>
										</div>
									<% } else { %>
										<div class="div_mypage_page_button">
											<a href="Controller?command=mypage_followingrecipe_view&page=<%=i%>"><%=i%></a>
										</div>
									<% } %>
								<% } %>
								...
								<% if(endPageNum < lastPageNum) { %>
									<div class="div_mypage_page_button">
										<a href="Controller?command=mypage_followingrecipe_view&page=<%=endPageNum+1%>">&gt;&gt;</a>
									</div>
								<% } %>
							</div>
						</div>
