...
<%
	ArrayList<RecipeVo> recipeList = (ArrayList<RecipeVo>) request.getAttribute("searchedRecipeList");   // 자-부 (X)
	...
	ArrayList<RecipeLvVo> getLv = (ArrayList<RecipeLvVo>) request.getAttribute("getLv");   // 자-부 (X)
	//ArrayList<RecipeVO> recipeList = (ArrayList<RecipeVO>) request.getAttribute("recipeList");   // 자-부 (X)
	RecipeBoardDao RBdDao = new RecipeBoardDao();
	...
	int searchedRecipeCount = (Integer) request.getAttribute("searchedRecipeCount");
	String swParam =request.getParameter("sw");
...
<head>
	...
	<link rel="stylesheet" href="Recipe_CSS/Recipe_Search_And_Classification.css" />
	...
	<script src="js/Recipe_SearchAndClassification.js"></script>
...
<!-- /페이지 상단 메뉴바 -->
	<!------------ header ------------>
	<dl class="container">
		<dd id="contents_area_full" style="clear:both;" class="search_table">
			<div id="search_category" class="rcp_m_cate">
						...
						<tr style="border-bottom: 1px solid #e6e7e8;">
							...
							<td>
								<div class=search_category_list>
									<div class="category_list" id ="what"><!--종류별-->
										<a class="whats">전체</a>
										<%
											for(RecipeManagerCategoryVo vo : getCategoryWhatList) {
										%>
										<a class="whats"><%=vo.getName()%></a>
										<%
											}
										%>
									</div>
									<div class="category_list" id ="kind"><!--유형별 -->
										<a class="kinds">전체</a>
										<%
											for(RecipeManagerCategoryVo vo : getCategoryKindList) {
										%>
										<a class="kinds"><%=vo.getName()%></a>
										<%
											}
										%>
									</div>
									<div class="category_list" id="situation"><!--상황별-->
										<a class="situations">전체</a>
										<%
											for(RecipeManagerCategoryVo vo : getCategorySituation) {
										%>
										<a class="situations"><%=vo.getName()%></a>
										<%
											}
										%>
									</div>
									<div class="category_list"><!-- 조리시간 -->
										<input type="number" id="min_time" min="5" max="120" style="width: 65px; padding: 0;" name="min-t" value=""/>
										<span>분 부터</span>
										<input type="number" id="max_time" min="5" max="100" style="width: 65px; padding: 0;" name="max-t" value=""/>
										<span>분 까지</span>
									</div>
									<div class="category_list"><!--난이도-->
										<select id="level" name="level" value=""id="lv">
											<option value="">미지정</option>
											<%
												for(RecipeLvVo vo : getLv) {
											%>
											<option value="<%=vo.getLv()%>"><%=vo.getLevelName()%></option>
											<%
												}
											%>
										</select>
			...
			<div class="rcp_cate_btn">
				<a id="show" class="search_category_bottom">
			...
			<ul class="search_result_container">
				...
				<%=searchedRecipeCount%>
					...
					<ul class="order_by fr">
						<li role="presentation" id="order_by_name">
							<a href="#">이름순</a>
						</li>
						<li role="presentation" id="order_by_recent">
							<a href="#">최신순</a>
						</li>
						<li role="presentation" id="order_by_hits">
							<a href="#">조회순</a>
						</li>
					</ul>
				</div>
				<ul class="ul_search_result_recipe_list">
 					<%
 						for(RecipeVo vo : recipeList) {
 					%>
						<li class="li_search_result_recipe_list">
							<div class="search_result_recipe_Thumbnail">
								<a href="Controller?command=view_recipe_detail&recipeID=<%=vo.getRecipeID()%>" class="SearchResultRecipe_link">
									<img src="<%=vo.getThumbnail()%>">
								</a>
							</div>
							<div class="search_result_recipe_caption_rv">
								<div class="search_result_recipe_caption_tit line2"><%=vo.getTitle()%></div>
								<div class="caption_user_profile">
									<a href="/MyPageRecipeView?uId="<%=vo.getUserID()%>>
										<img src="<%=vo.getProfileImage()%>"><%=vo.getNickname()%>
									</a>
								</div>
								<div class="search_result_recipe_caption">
									<% 
									if(vo.getCommentCnt() != 0 ){
									%>
										<span class="search_result_recipe_caption_star">
									<% 
										for(int j = 1; j <= 5; j++) {
									%>
									...
									<span class="search_result_recipe_caption_rv_cnt">(<%=vo.getCommentCnt()%>)</span>
									<% 
									}
									%>
									<span class="search_result_recipe_caption_rv_hits">조회수 <%=vo.getHits()%></span>
								</div>
							</div>
						</li>
					<%
				...
				<nav class="text-center">
					<ul class="pagination">
						<% if(startPNum>1) { %>
							<li>
								<a href="Controller?command=recipe_search&page=<%=startPNum-1%>">&lt;&lt;</a>
							</li>
						<% } %>
						<% for(int i=startPNum; i<=lastPageNum; i++) { %>
						<%	if(i==pageNum) { %>
							<li class="now_page">
								<a><%=i %></a>
							</li>
							<%  } else{  %>
								<li>
									<a href="Controller?command=recipe_search&page=<%=i%>"><%=i %></a>
								</li>
							<%  } %>
						<% } %>
						<% if(endPNum+1 <= lastPageNum) { %>
							<li>
								<a href="Controller?command=recipe_search&page=<%=lastPageNum+1%>">&gt;&gt;</a>
							</li>
						<% } %>
					</ul>
				</nav>
