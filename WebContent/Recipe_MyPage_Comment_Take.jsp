...
<%
	...
	// 로그인 계정의  레시피에 달린 댓글 정보에 필요한 변수 선언
	ArrayList<MyPageCommentViewDto> receiveCommentList = null;
	...
	if(loginId != null) {
		...
		// 로그인 계정의 레시피에 달린 댓글 리스트
		receiveCommentList = (ArrayList<MyPageCommentViewDto>)request.getAttribute("receiveCommentList");
	}
...
<html>
<head>
	<!-- 마이페이지 본인계정일 때의 후기 페이지(내가 쓴 요리 후기 - 아무것도 없을 때) -->
	<meta charset="UTF-8">
	...
	<link href="Recipe_CSS/Recipe_MyPage_Comment_Take.css" rel="stylesheet">
...
<!-- 마이페이지 헤더 -->
			<div id="div_mypage_header">
				...
				<div class="div_mypage_header_button">
					<a href="Controller?command=mypage_comment_view" class="button-active">
...
<!-- /마이페이지 헤더 -->
<!-- 메인컨텐츠(마이페이지 댓글 내가 받은거) -->
			<div id="div_mypage_comment_panel">
				<div id="div_mypage_comment_sort">
					<div id="div_mypage_comment_sort1">
						<div>
							<a href="Controller?command=mypage_comment_view&mode=receive" class="open_active">받은 요리 후기</a>
						</div>
						<div>
							<a href="Controller?command=mypage_comment_view&mode=send">내가 쓴 요리 후기</a>
						</div>
					</div>
				</div>
				<% if(loginId != null) { %>
					<% if(receiveCommentList.isEmpty()) { %>
					...
					<% } else { %>
						<div id="div_mypage_comment_body">
							<div id="div_mypage_comment_content">
								<% for(MyPageCommentViewDto commentViewObj : receiveCommentList) { %>
									<%
								...
								<div id="div_mypage_page_bundle">
									<% if(startPageNum != 1) { %>
										<div class="div_mypage_page_button">
											<a href="Controller?command=mypage_comment_view&mode=receive&page=<%=startPageNum-3%>">&lt;&lt;</a>
										</div>
									<% } %>
									<% if(pageNum != 1) { %>
										<div class="div_mypage_page_button">
											<a href="Controller?command=mypage_comment_view&mode=receive&page=<%=pageNum-1%>">&lt;</a>
										</div>
									<% } %>
										<% for(int i=startPageNum; i<=endPageNum; i++) { %>
											...
											<% if(pageNum == i) { %>
												<div class="div_mypage_page_button">
													<a href="Controller?command=mypage_comment_view&mode=receive&page=<%=i%>" class="button_active"><%=i%></a>
												</div>
											<% } else { %>
												<div class="div_mypage_page_button">
													<a href="Controller?command=mypage_comment_view&mode=receive&page=<%=i%>"><%=i%></a>
												</div>
											<% } %>
										<% } %>
									...
									<% if(lastPageNum > endPageNum) { %>
										<div class="div_mypage_page_button">
											<a href="Controller?command=mypage_comment_view&mode=receive&page=<%=endPageNum+1%>">&gt;&gt;</a>
										</div>
									<% } %>
								</div>
							</div>
