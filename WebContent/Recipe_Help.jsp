...
<%
	...
	// 공지사항 목록 가져오기
	ArrayList<HelpDto> helpList = (ArrayList<HelpDto>)request.getAttribute("helpList");
	// 우측 하단 베스트상품 5종 가져오기
...
<head>
	...
	<link href="Recipe_CSS/Recipe_Help.css" rel="stylesheet">
...
<!-- 도움말 -->
			<div id="div_panel">
				<% for(HelpDto helpObj : helpList) { %>
					<%
						int helpId = helpObj.getHelpId();
						String helpTitle = helpObj.getHelpTitle();
						String helpContent = helpObj.getHelpContent();
						String helpWritedate = helpObj.getHelpWritedate();
					%>
					<div class="cs_content" help="<%=helpId%>">
						<div class="cs_post_header">
							<a>
								...
								<span class="date"><%=helpWritedate%></span>
							</a>
						</div>
				...
				<div class="cs_guide">
					<span>
						원하시는 답변을 찾지 못하셨다면, <a href="/customer/list.html?bid=3" style="text-decoration:underline;color:#666;">여기</a>에 문의하여주세요. 
					</span>
				</div>
			</div>
<!-- /도움말 -->
...
<!-- 고객센터 목록 -->
			<div id="div_cs_list">
				...
				<a href="Controller?command=help_view" class="cs_name_button on">도움말</a>
