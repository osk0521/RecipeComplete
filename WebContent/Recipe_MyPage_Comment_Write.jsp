...
<%
	...
	// 로그인 계정의 작성 댓글 정보에 필요한 변수 선언
	ArrayList<MyPageCommentViewDto> myCommentList = null;
	...
	if(loginId != null) {
		...
		// 로그인 계정의 작성 댓글 리스트
		myCommentList = (ArrayList<MyPageCommentViewDto>)request.getAttribute("myCommentList");
	}
...
<head>
	...
	<link href="Recipe_CSS/Recipe_MyPage_Comment_Write.css" rel="stylesheet">
			...
			<div id="div_mypage_comment_panel">
				<div id="div_mypage_comment_sort">
					<div id="div_mypage_comment_sort1">
						<div>
							<a href="Controller?command=mypage_comment_view&mode=receive">받은 요리 후기</a>
						</div>
						<div>
							<a href="Controller?command=mypage_comment_view&mode=send" class="open_active">내가 쓴 요리 후기</a>
						</div>
					</div>
				</div>
				<% if(loginId != null) { %>
					...
					<% } else { %>
						<div id="div_mypage_comment_body">
							<div id="div_mypage_comment_content">
								<% for(MyPageCommentViewDto commentViewObj : myCommentList) { %>
									<%
								...
								<div id="div_mypage_page_bundle">
									<% if(startPageNum != 1) { %>
										<div class="div_mypage_page_button">
											<a href="Controller?command=mypage_comment_view&mode=send&page=<%=startPageNum-3%>">&lt;&lt;</a>
										</div>
									<% } %>
									<% if(pageNum != 1) { %>
										<div class="div_mypage_page_button">
											<a href="Controller?command=mypage_comment_view&mode=send&page=<%=pageNum-1%>">&lt;</a>
										</div>
									<% } %>
										<% for(int i=startPageNum; i<=endPageNum; i++) { %>
											...
											<% if(pageNum == i) { %>
												<div class="div_mypage_page_button">
													<a href="Controller?command=mypage_comment_view&mode=send&page=<%=i%>" class="button_active"><%=i%></a>
												</div>
											<% } else { %>
												<div class="div_mypage_page_button">
													<a href="Controller?command=mypage_comment_view&mode=send&page=<%=i%>"><%=i%></a>
												</div>
											<% } %>
										<% } %>
									...
									<% if(lastPageNum > endPageNum) { %>
										<div class="div_mypage_page_button">
											<a href="Controller?command=mypage_comment_view&mode=send&page=<%=endPageNum+1%>">&gt;&gt;</a>
										</div>
									<% } %>
								</div>
							</div>
