...
<%@page import="dto.InquiryAnswerDto"%>
...
<%
	...
	// 로그인 유저의 문의사항 목록 가져오기
	ArrayList<InquiryDto> inquiryList = (ArrayList<InquiryDto>)request.getAttribute("inquiryList");
	// 우측 하단 베스트상품 5종 가져오기
...
<head>
	...
	<link href="Recipe_CSS/Recipe_Inquiry.css" rel="stylesheet">
...
<!-- 문의사항 등록 -->
			<div id="div_inquiry_reg">
				<div id="div_inquiry_reg_title">
					<p>
						<span>
							궁금하신 점이나 제안하고 싶은 점이 있으시면 언제든지 글을 남겨주세요.<br/>
						</span>
						<span>
							(휴일 문의건에 대해서는 휴무일 이후 순차적으로 처리해드리겠습니다)
						</span>
					</p>
				</div>
				<div id="div_inquiry_reg_button">
					<a href="Controller?command=inquiry_write_form">등록</a>
				</div>
			</div>
<!-- /문의사항 등록 -->
			<div class="blank_space"></div>
<!-- 메인컨텐츠(1:1문의사항) -->
			<div id="div_inquiry_panel">
				<% if(loginId == null) { %>
<!-- 1:1문의사항 비로그인 -->
					<div id="div_inquiry_none">
						<img src="https://recipe1.ezmember.co.kr/img/none_login.png"/>
						<p>
							로그인하시면 Q&amp;A 리스트에서<br>
							질문과 답변 내용을 확인하실 수 있습니다.
						</p>
						<button type="button" onclick='location.href="Controller?command=login_form"'>로그인하기</button>
					</div>
<!-- /1:1문의사항 비로그인 -->
				<% } else { %>
					<% for(InquiryDto inquiryObj : inquiryList) { %>
						<%
							int inquiryId = inquiryObj.getInquiryId();
							String inquiryTitle = inquiryObj.getInquiryTitle();
							String inquiryContent = inquiryObj.getInquiryContent();
							String inquiryWriter = inquiryObj.getInquiryWriter();
							String inquiryWritedate = inquiryObj.getInquiryWritedate();
							InquiryAnswerDto inquiryAnswer = inquiryObj.getInquiryAnswer();
						%>
						<% if(inquiryAnswer == null) { %>
							<div class="div_inquiry_content" inquiry="<%=inquiryId%>">
								<div class="div_inquiry_header">
									<a class="a_inquiry">
										<div class="div_inquiry_recipt">접수</div>
										<div class="div_inquiry_title">
											<%=inquiryTitle%>
											<div>
												<img src="https://recipe1.ezmember.co.kr/img/icon_arrow1_down.png"/>
											</div>
										</div>
										<div class="div_inquiry_date"><%=inquiryWritedate.substring(0, inquiryWritedate.length()-3)%></div>
									</a>
								</div>
								<div class="div_inquiry_body">
									<div class="div_inquiry_content">
										<%=inquiryContent%>
									</div>
									<div class="div_inquiry_button">
										<button class="button_inquiry_button modify">수정</button>
										<button class="button_inquiry_button delete">삭제</button>
									</div>
								</div>
							</div>
							<div class="blank_space"></div>
						<% } else { %>
							<%
								String inquiryAnswerContent = inquiryAnswer.getInquiryAnswerContent();
								String inquiryAnswerWritedate = inquiryAnswer.getInquiryAnswerWritedate();
							%>
							<div class="div_inquiry_content" inquiry="<%=inquiryId%>">
								<div class="div_inquiry_header">
									<a class="a_inquiry">
										<div class="div_inquiry_recipt">답변완료</div>
										<div class="div_inquiry_title">
											<%=inquiryTitle%>
											<div>
												<img src="https://recipe1.ezmember.co.kr/img/icon_arrow1_down.png"/>
...
<!-- 고객센터 목록 -->
			<div id="div_cs_list">
				<a href="Controller?command=notice_view" class="cs_name_button">공지사항</a>
				<a href="Controller?command=help_view" class="cs_name_button">도움말</a>
				<a href="Controller?command=inquiry_view" class="cs_name_button on">문의하기</a>
			</div>
<!-- /고객센터 목록 -->
			<div class="blank_space"></div>
<!-- 사이트 자체 광고 -->
