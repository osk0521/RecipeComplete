...
<%@page import="dao.RecipeRankBoardDao"%>
<%@page import="java.util.ArrayList"%>
<%@ page import="dto.ProfileDto"%>
<%
	String rtype = (String)(request.getParameter("rtype"));
	String ptype = (String)(request.getParameter("ptype"));
	ArrayList<RecipeRankVo> top100RecipeList = (ArrayList<RecipeRankVo>) request.getAttribute("top100RecipeList"); 
	ArrayList<RecipeUserRankVo> top100UserList = (ArrayList<RecipeUserRankVo>) request.getAttribute("top100UserList");
	ArrayList<String> top100Searchword = (ArrayList<String>) request.getAttribute("top100Searchword");  
	RecipeRankBoardDao RRBDao = new RecipeRankBoardDao();
	//int pageNum = (Integer) request.getAttribute("");
...
<head>
	...
	<link rel="stylesheet" href="Recipe_CSS/Recipe_Ranking.css"/>
	<link rel="stylesheet" href="Recipe_CSS/Public/Recipe_Header.css"/>
	<link rel="stylesheet" href="Recipe_CSS/Public/Recipe_Footer.css"/>
	<link rel="stylesheet" href="slick-jw/slick_jw.css"/>
	<link rel="stylesheet" href="slick-jw/slick-theme_jw.css"/>
	<script src="js/jquery-3.7.1.min.js"></script>
	<script src="js/Recipe_Ranking.js"></script>
...
<!-- /페이지 상단 메뉴바 -->
<!------------ header ------------>
	<dl class="container sub_bg">
		<dd id="contents_area_full">
		<div class="searchRank">
			<div class="rcp_m_list2">
				<ul class="search_word_object">
					<li role="presentation" class="active rtype" rtype="r">
						<a>레시피</a>
					</li>
					<li role="presentation" class="non-active rtype" rtype="k">
						<a>검색어</a>
					</li>
					<li role="presentation" class="non-active rtype" rtype="c">
 						<a>쉐프</a>
					</li>
					<ul class="peroiod">
						<li role="presentation" class="active ptype" ptype="d">
							<a>일간</a>
						</li>
						<li role="presentation" class="non-active ptype" ptype="w">
							<a>주간</a>
						</li>
						<li role="presentation" class="non-active ptype" ptype="m">
							<a>월간</a>
						</li>
					</ul>
				</ul>
				...
				<!-- 게시글 -->
				<ul class="rank_recipe">
					<%
						int i = 1;
						for(RecipeRankVo vo : top100RecipeList) {
					%>
					<li class="rank_recipe_list">
						<p class="ranking_num recipe">
							<b><%=i%></b>
						</p>
						...
						<div class="rank_caption_tit line2"><%=vo.getTitle()%></div>
						<div class="rank_caption_rv_name">
							<a>
								<img src="<%=vo.getProfileImage()%>"><%=vo.getNickname()%>
							</a>
						</div>
						<div class="rank_caption_rv">
						<%
						...
						%>
							...
							<span class="rank_caption_rv_ea">(<%=vo.getCommentCnt()%>)</span>
						<%
							}
						%>
						<span class="rank_caption_hits">조회수 <%=vo.getHits()%></span>
						</div>
						</div>
					</li>
					<%
				...
				<!-- 검색어 -->
				<ul class="ranking_search_word">
					<%
						i = 1;
						for(int k = 0; i<= top100Searchword.size(); k++) {
					%>
					<li>
						<div class="search_word_list">
							...
							<a href="Controller?command=recipe_search&sw=<%=top100Searchword.get(k)%>"><%=top100Searchword.get(k)%></a>
						</div>
					</li>
					<%
						++i;
				...
				<!-- 유저 -->
			<div class="row">
				<ul class="ranker_user_list">
						<%
							i = 1;
							for(RecipeUserRankVo vo : top100UserList) {
						%>
					<li>
						<div class="ranker_user_thumbnail">
							<p class="ranking_num">
								<b><%=i%></b>
							</p>
							<a href="Controller?command=mypage_recipe_view&uId=<%=vo.getUserID()%>">
								<img src="<%=vo.getProfile()%>"/>
							</a>
						</div>
						<div class="ranker_user_nickname"><%=vo.getNickname()%></div>
					</li>
					<%
					++i;
					}
