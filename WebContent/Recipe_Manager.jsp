...
<%@page import="vo.RecipeManagerUserVo"%>
<%@page import="vo.RecipeManagerUserVo"%>
<%
	...
	ArrayList<RecipeManagerUserVo> listForUser = (ArrayList<RecipeManagerUserVo>) request.getAttribute("getUserlist");  
	ArrayList<RecipeManagerCategoryVo> listForWhat = (ArrayList<RecipeManagerCategoryVo>) request.getAttribute("getlistForWhat");  
	ArrayList<RecipeManagerCategoryVo> listForKind = (ArrayList<RecipeManagerCategoryVo>) request.getAttribute("getlistForKind");  
	ArrayList<RecipeManagerCategoryVo> listForSituation = (ArrayList<RecipeManagerCategoryVo>) request.getAttribute("getlistForSituation");   
...
<head>
	...
	<link rel="stylesheet" href="Recipe_CSS/Recipe_Manager.css"/>
	...
	<script src="js/Recipe_Manager.js"></script>
	...
	%>
		alert("매니저 아닌 멤버는 사용할 수 없는 페이지 입니다.");
	...
	<script>
		/* for(RecipeManagerUserVo rmuvo : listForUser) {
			UserList.push(
				{reg_date: rmuvo., nickname: rmuvo.getNickname(),  b_date: "20170101", u_id: rmuvo.getUserID(), phone: rmuvo.getPhoneNum(), seller:"일반회원"}
			);
		} */
		/* for (var i = 0; i < 50; i++) {
			RecipePostList.push(
		...
		for (RecipeManagerCategoryVo rmcvo : listForWhat) {
			SearchTermsList.push(
				{what: rmcvo.getCategoryId(), name: rmcvo.getName()}
			);
		}
		for (RecipeManagerCategoryVo rmcvo : listForKind) {
			SearchTermsList.push(
				{what: rmcvo.getCategoryId(), name: rmcvo.getName()}
			);
		}
		for (RecipeManagerCategoryVo rmcvo : listForSituation) {
			SearchTermsList.push(
				{what: rmcvo.getCategoryId(), name: rmcvo.getName()}
			);
		}
	...
	<dd class="container">
	  <!--쇼핑베스트-->
		<div class="body_content">
			 <div class="Manager SearchTermsList hidden" id="editMainpage" data-ax5grid="MainpageGrid" data-ax5grid-config="{}">
			</div>
			<div class="Manager SearchTermsList hidden" id="SearchTermsList" data-ax5grid="SearchTermsGrid" data-ax5grid-config="{}">
			</div>
			<div class="Manager RecipePostList hidden" id="RecipePost" data-ax5grid="RecipePostGrid" data-ax5grid-config="{}">
			</div> 
			<div class="Manager UserList" id="UserList" data-ax5grid="UserGrid" data-ax5grid-config="{}">
			</div>
			<div class="Manager CSList hidden" id="CSList" data-ax5grid="CSGrid" data-ax5grid-config="{}">
			</div>
			<div class="Manager MagazineList hidden" id="MagazineList" data-ax5grid="MagazineGrid" data-ax5grid-config="{}">
			</div>
		</div>
		<div style="display: flex; justify-content: flex-end">
			<button class ="edit_btn" data-grid-control="row-add" id="add_data">데이터 추가</button>
			<button class ="edit_btn" data-grid-control="row-remove" id="delete_data">데이터 삭제</button>
			<button class ="edit_btn" data-grid-control="row-update" id ="btn_modify">저장</button>
		</div>
	</dd>
<!------------ body ------------>
<!-- footer 흰색배경부분 -->
